...
#include <unistd.h>
#else
#error "RX7900XTX benchmark harness currently requires Linux."
#endif
...
static std::string find_json_result(
    ...
    while(std::getline(stream,line)){
        if(
            line.find("\"status\":")
                !=std::string::npos
            &&line.find("\"device\":\"rx7900xtx-24g\"")
                !=std::string::npos
        ){
            found=line;
        }
...
static Options parse_cli(
    ...
    if(options.binary.empty()){
        ...
        options.binary=
            self.parent_path()
            /(
                "qingming-qwen3-tts_rx7900xtx-24g_"
                +options.family
            );
    }
...
static int run(
    ...
    std::cout
        ...
        <<"Resident split: 28 WGP generation / 20 WGP decoder\n\n";
    ...
    text_report
        <<std::fixed
        <<std::setprecision(3)
        <<"Qingming Qwen3-TTS RX7900XTX-24G Benchmark Report\n"
        <<"==================================================\n"
        <<"Family: "<<options.family<<"\n"
        <<"Task: "<<options.task<<"\n"
        <<"Text mode: "<<options.text_mode<<"\n"
        ...
        <<"Resident WGP split: generation 28 / decoder 20\n"
        <<"Seed: "<<options.seed<<"\n"
