...
static CliOptions parse_cli(
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--model-dir") {
            ...
            std::cout
                << "Qwen3-TTS 1.7B RX7900XTX-24G C++/HIP runtime\n\n"
                << "Usage:\n  "
                << argv[0]
                << " --model-dir DIR"
...
public:
    ...
    PersistentFrameResult run_frame_device(
        ...
        // The 2048-d next Talker vector itself never leaves the GPU.
        PRED_HIP_CHECK(
            hipStreamSynchronize(
                stream_));
...
public:
    ...
    void decode_device(
        ...
        for (
            ...
            //
            // M=1 OProj uses the LDS-staged linear so the 2048-d context
...
public:
    ...
    void zero_async(hipStream_t stream){
        if(p_&&n_){
            hipok(hipMemsetAsync(p_,0,n_,stream),"hipMemsetAsync");
        }
    }
...
public:
    StreamingCodecDecoder(
      ...
      :P_(std::move(P)),
       ...
       owns_stream_(resident_stream==nullptr){
        ...
        allocate();
        reset_workspace();
        hipok(hipStreamSynchronize(stream_),"streaming decoder allocation/reset sync");
    ...
    void reset_request(){
        reset_workspace();
        hipok(hipStreamSynchronize(stream_),"streaming decoder request reset sync");
    }
...
public:
    ResidentEngine(
        ...
        std::cout
            ...
            <<(g_resident_cu_partition_enabled?"True":"False")<<"\n"
            <<"resident_generation_wgp_count: "
            <<(g_resident_cu_partition_enabled?g_resident_generation_wgps:48)<<"\n"
            <<"resident_decoder_wgp_count: "
            <<(g_resident_cu_partition_enabled?g_resident_decoder_wgps:48)<<"\n"
