// RTX4090-24G Qwen3-TTS 0.6B execution family.
...
// source rename.
//
// Initial RTX4090-24G resident production partition:
//   generation 80 SM
//   decoder    48 SM
// ============================================================================
...
//
// Runtime:
//   one C++/CUDA process, no Python/HuggingFace/cuBLAS/cuBLASLt/CUTLASS bridge.
//
...
static CliOptions parse_cli(
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--model-dir") {
            ...
            std::cout
                << "Qwen3-TTS legacy_legacy_tag_j.3 official Base x-vector voice clone, single C++/CUDA\n\n"
                << "Usage:\n  "
                << argv[0]
                << " --model-dir DIR"
                << " --ref-audio ref.wav"
                ...
                << "  frontend-native        Native tokenizer + safetensors + CUDA text projection/prefill;\n"
...
//
// AD102 / RTX 4090 exposes 128 SMs and warp size 32.  The initial resident
...
// shared memory, then eight warp32 groups each compute four logits. Every output keeps
...
__device__ __forceinline__
float final_head_processed_score(
    float accumulated,
    int token,
    const std::uint8_t* __restrict__ seen_tokens,
    int generation_index) {

    const std::uint16_t bf16_score=
        qwen3_tts::native_ops::
            float_to_bf16_rne(accumulated);

    float score=
        legacy_bf16_to_float(
            bf16_score);

    if(seen_tokens[token]!=0){
        const float reciprocal=
            __uint_as_float(0x3f73cf3eu);
...
// The production FinalHead keeps the codec-head BF16 boundary and consumes it
// immediately for repetition and token filtering. The 1024-d hidden row is
// staged once per block and each warp computes four vocabulary outputs.
__global__ __launch_bounds__(256, 2)
void final_head_06b_pack4_process_kernel(
    const std::uint16_t* __restrict__ hidden,
    const std::uint16_t* __restrict__ weight,
    const std::uint8_t* __restrict__ seen_tokens,
    int generation_index,
    ...
    for(
        int column=static_cast<int>(threadIdx.x);
        column<hidden_size;
        column+=static_cast<int>(blockDim.x)
    ){
        shared_hidden[column]=hidden[column];
    }
    ...
    const bool have1=out1<kCodecVocab;
    const bool have2=out2<kCodecVocab;
    const bool have3=out3<kCodecVocab;

    const std::size_t base0=
        static_cast<std::size_t>(out0)*hidden_size;
    const std::size_t base1=
        have1?static_cast<std::size_t>(out1)*hidden_size:0;
    const std::size_t base2=
        have2?static_cast<std::size_t>(out2)*hidden_size:0;
    const std::size_t base3=
        have3?static_cast<std::size_t>(out3)*hidden_size:0;
    ...
    for(int k=lane;k<hidden_size;k+=32){
        ...
        a0=fmaf(
            x,
            legacy_bf16_to_float(weight[base0+k]),
            a0);

        if(have1){
            a1=fmaf(
                x,
                legacy_bf16_to_float(weight[base1+k]),
                a1);
        }
        ...
        if(have2){
            a2=fmaf(
                x,
                legacy_bf16_to_float(weight[base2+k]),
                a2);
        }
        ...
        if(have3){
            a3=fmaf(
                x,
                legacy_bf16_to_float(weight[base3+k]),
                a3);
        }
    }

    a0=qwen3_tts::native_ops::warp_reduce_sum(a0);
    if(have1)a1=qwen3_tts::native_ops::warp_reduce_sum(a1);
    if(have2)a2=qwen3_tts::native_ops::warp_reduce_sum(a2);
    if(have3)a3=qwen3_tts::native_ops::warp_reduce_sum(a3);

    if(lane==0){
        processed[out0]=
            final_head_processed_score(
                a0,out0,seen_tokens,generation_index);

        if(have1){
            processed[out1]=
                final_head_processed_score(
                    a1,out1,seen_tokens,generation_index);
        }

        if(have2){
            processed[out2]=
                final_head_processed_score(
                    a2,out2,seen_tokens,generation_index);
        }

        if(have3){
            processed[out3]=
                final_head_processed_score(
                    a3,out3,seen_tokens,generation_index);
        }
    }
}
...
public:
    explicit NativeFinalHead(
        ...
        std::cout
            ...
            <<"final_head_06b_device_backend: shared_pack4_process_3072x1024\n"
            <<"final_head_06b_hidden_shared_bytes: 2048\n"
            <<"final_head_06b_projection_process_fused: True\n"
            <<"final_head_repetition_seen_map: True\n";
    }
    ...
    std::int32_t run_fast_device(
        ...
        // The fused producer retains the BF16 codec-head boundary before
        // applying repetition and token filtering.
        profile_total_.begin(stream);
        profile_projection_process_.begin(stream);

        QINGMING_CUDA_LAUNCH(
            final_head_06b_pack4_process_kernel,
            dim3(
                static_cast<unsigned>(
                    (kCodecVocab+32-1)/32)),
            dim3(256),
...
public:
    ...
    void decode_device(
        ...
        for (
            ...
            //
            // M=1 OProj uses the shared memory-staged linear so the 1024-d context
...
static int run_official_base_xvector(
    ...
    std::cout
        ...
        <<"legacy_runtime_external_blas_dependency_b: False\n"
...
public:
    ResidentEngine(
        ...
        std::cout
            ...
            <<maximum_frames_
            <<"\n"
            <<"resident_generation_profile_enabled: "
            <<(g_generation_profile_enabled?"True":"False")
            <<"\n"
            ...
            <<"resident_speaker_embedding_cache_enabled: "
            <<(task_=="base-xvector"?"True":"False")
            <<"\n"
            <<"resident_speaker_embedding_cache_capacity: "
            <<(task_=="base-xvector"?kResidentSpeakerCacheCapacity:0)
            <<"\n"
...
private:
    ...
    std::vector<std::uint16_t> base_speaker_embedding(
        ...
        if(embedding.size()!=1024){
            throw std::runtime_error(
                "Base resident speaker embedding must contain 1024 BF16 values");
        }
